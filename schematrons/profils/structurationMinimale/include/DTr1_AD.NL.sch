...
<rule xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="AD.NL">
   ...
   <assert role="error" test="if (@codeSystem='2.16.840.1.113883.2.4.4.15') then (matches(@code,'^[0-9]{4}[A-Z]{2}$')) else ( true() )" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.NL">dtr1-1-AD.NL: postcodes onder codeSystem '2.16.840.1.113883.2.4.4.15' moeten voldoen aan '9999AA'</assert>
   <assert role="error" test="if (hl7:postalCode/text() and $isDutchAddress)             then ( matches($postalCodeText,'^[0-9]{4} [A-Z]{2}$'))            else ( true() )" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.NL">dtr1-2-AD.NL: Nederlandse postcodes in de text-nodes moeten voldoen aan '9999 AA'</assert>
   ...
   <assert role="error" test="not($isDutchAddress) or empty($illegalAddressPart)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.NL">dtr1-3-AD.NL: binnen Nederland mogen alleen delimiter, country, county, city, postalCode, houseNumber, buildingNumberSuffix, streetName, additionalLocator, unitID en useablePeriod worden gebruikt. Gevonden '<value-of select="$illegalAddressPart"/>'.</assert>
   <let name="illegalCodedAddressPart" value="*[not(self::hl7:postalCode|self::hl7:country|self::hl7:county)][@code]/name()"/>
   <assert role="error" test="not($isDutchAddress) or empty($illegalCodedAddressPart)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.NL">dtr1-4-AD.NL: alleen postalCode, country en county kunnen gecodeerd worden. Gevonden '<value-of select="$illegalCodedAddressPart"/>'</assert>
</rule>