...
<rule xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="AD.CA">
   ...
   <assert role="error" test="@nullFlavor or (translate(concat(' ', @use, ' '), '(H|PST|TMP|WP) ', '') = '' and (count(tokenize (@use, ' '))&lt;=3))" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.CA">dtr1-1-AD.CA: @use may have up to three codes and must be drawn from 'H', 'PST', 'TMP', 'WP'</assert>
   <assert role="error" test="@nullFlavor or count(* except (hl7:country | hl7:city | hl7:state | hl7:postalCode | hl7:streetAddressLine))=0 or count(hl7:delimiter)&lt;=4" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.CA">dtr1-2-AD.CA: supported address parts (part types) are country, city, state, postalCode, streetAddressLine and up to four delimiter</assert>
   <assert role="error" test="@nullFlavor or count(*[@code])=0" see="https://docs.art-decor.org/documentation/datatypes/DTr1_AD.CA">dtr1-3-AD.CA: address parts SHALL NOT contain @code</assert>
</rule>