...
<rule xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" abstract="true" id="IVL_TS">
   ...
   <assert role="error" test="not(descendant-or-self::*[@nullFlavor and (@value|@unit|hl7:low|hl7:center|hl7:high|hl7:width)])" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-2-IVL_TS: null violation. Cannot have @nullFlavor and @value or other child elements</assert>
   <assert role="error" test="not(@value and hl7:*)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-3-IVL_TS: co-occurence violation. Cannot have @value and other child elements</assert>
   <!-- unsure where this rule comes from KH 2016-07-22
    <assert role="error" test="@nullFlavor or descendant-or-self::*[@value]" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS"
        ...
        -->
   <assert role="error" test="not(@value and (hl7:low|hl7:center|hl7:high|hl7:width))" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-4-2-IVL_TS: co-occurence violation. Cannot have @value and child elements</assert>
   <assert role="error" test="not(hl7:center and (hl7:low|hl7:high|hl7:width))" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-4-3-IVL_TS: co-occurence violation. Cannot have center and other elements</assert>
   <assert role="error" test="not(hl7:width and hl7:low and hl7:high)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-4-4-IVL_TS: co-occurence violation. Cannot have width and have both low and high elements</assert>
   <assert role="error" test="not(hl7:*[@updateMode])" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-5-IVL_TS: no updateMode on IVL attributes</assert>
   <assert role="error" test="not(hl7:low/@value = hl7:high/@value)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-6-IVL_TS: low/@value must not be equal to high/@value</assert>
   <!-- width has datatype PQR, which extends CV ((){1}(((0[1-9])|([12]\d)|(3[01]))?)?)? -->
   <assert role="error" test="not(hl7:width[@unit][not(@value)])" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-1-PQR: width element: no unit without value</assert>
   <assert role="error" test="not(hl7:width/hl7:translation)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-2-PQR: width element: no translation</assert>
   <assert role="error" test="not(hl7:low[@nullFlavor='PINF'])" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-7-1-IVL_TS: low must be lower than or equal to high. Found low boundary PINF (Positive Infinity)</assert>
   <assert role="error" test="not(hl7:high[@nullFlavor='NINF'])" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-7-2-IVL_TS: low must be lower than or equal to high. Found high boundary NINF (Negative Infinity)</assert>
   ...
   <assert role="error" test="matches(hl7:width/@unit, $tum)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-8-IVL_TS: for width only us (microseconds), ms (milliseconds), s (seconds), min (minute), h (hours), d (day), wk (week), mo (month) or a (year) are allowed</assert>
   ...
   <assert role="error" test="empty($theTSLow) or ($theTSLow=$theTSStringLow and $theLowDateTime castable as xs:dateTime)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-9-IVL_TS: <value-of select="local-name()"/>/low "<value-of select="$theTSLow"/>" is not a valid timestamp.</assert>
   ...
   <assert role="error" test="empty($theTSCenter) or ($theTSCenter=$theTSStringCenter and $theCenterDateTime castable as xs:dateTime)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-9-IVL_TS: <value-of select="local-name()"/>/center "<value-of select="$theTSCenter"/>" is not a valid timestamp.</assert>
   ...
   <assert role="error" test="empty($theTSHigh) or ($theTSHigh=$theTSStringHigh and $theHighDateTime castable as xs:dateTime)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-9-IVL_TS: <value-of select="local-name()"/>/high "<value-of select="$theTSHigh"/>" is not a valid timestamp.</assert>
   <assert role="error" test="empty($theTSLow) or empty($theTSHigh) or not($theLowDateTime castable as xs:dateTime) or not($theHighDateTime castable as xs:dateTime) or xs:dateTime($theLowDateTime) le xs:dateTime($theHighDateTime)" see="https://docs.art-decor.org/documentation/datatypes/DTr1_IVL_TS">dtr1-7-IVL_TS: low/@value (<value-of select="$theTSLow"/>) must be before high/@value (<value-of select="$theTSHigh"/>)</assert>
</rule>