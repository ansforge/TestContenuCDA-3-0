...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" id="Entete_AVC-PAVC">
    ...
    <rule context="cda:ClinicalDocument">
        <assert test="cda:templateId[@root='1.2.250.1.213.1.1.1.25']"> 
            [Entete_AVC-PAVC] Le template du modèle AVC-PAVC (1.2.250.1.213.1.1.1.25) doit être présent.
        </assert>
        
        <assert test="./cda:code[@code='11488-4' and @codeSystem='2.16.840.1.113883.6.1']"> 
            [Entete_AVC-PAVC]  L'élément code doit avoir @code ="11488-4" et @codeSystem = "2.16.840.1.113883.6.1"/>. 
        </assert>
        <assert test="./count(cda:setId[@root])=1"> 
            [Entete_AVC-PAVC]  Erreur de conformité au modèle : L'élément "setId" doit être présent. 
        </assert>
        <assert test="./count(cda:versionNumber[@value])=1"> 
            [Entete_AVC-PAVC] Erreur de conformité au modèle : L'élément "versionNumber" doit être présent. 
        </assert>
    </rule>
</pattern>