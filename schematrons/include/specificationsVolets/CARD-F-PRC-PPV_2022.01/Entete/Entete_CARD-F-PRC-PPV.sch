...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" id="Entete_CARD-F-PRC-PPV">
    ...
    <rule context="cda:ClinicalDocument">
        ...
        <assert test="./count(cda:setId[@root])=1"> 
            [Entete_CARD-F-PRC-PPV] Erreur de conformité : L'élément "setId" doit être présent. 
        </assert>
        <assert test="./count(cda:versionNumber[@value])=1"> 
            [Entete_CARD-F-PRC-PPV] Erreur de conformité : L'élément "versionNumber" doit être présent. 
        </assert>
    </rule>
    <rule context="cda:ClinicalDocument/cda:participant">
        <assert test="./@typeCode='INF'"> 
            [Entete_CARD-F-PRC-PPV] Erreur de conformité au modèle : L'élément "participant" doit avoir l'attribut @typeCode='INF'.
        </assert>
        <assert test="./cda:functionCode[@code='PCP' or @code='ATTPHYS']"> 
            [Entete_CARD-F-PRC-PPV] Erreur de conformité au modèle : L'élément "participant/functionCode" doit avoir l'attribut @code='PCP' pour le médecin traitant et @code='ATTPHYS' pour le Cardiologue traitant.
        </assert>
    </rule>
