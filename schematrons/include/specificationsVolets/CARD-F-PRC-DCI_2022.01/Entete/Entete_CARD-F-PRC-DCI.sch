...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" id="Entete_CARD-F-PRC-DCI">
    ...
    <rule context="cda:ClinicalDocument/cda:participant">
        <assert test="./@typeCode='INF'"> 
            [Entete_CARD-F-PRC-DCI] Erreur de conformité au modèle : L'élément "participant" doit avoir l'attribut @typeCode='INF'.
        </assert>
        <assert test="./cda:functionCode[@code='PCP' or @code='ATTPHYS']"> 
            [Entete_CARD-F-PRC-DCI] Erreur de conformité au modèle : L'élément "participant/functionCode" doit avoir l'attribut @code='PCP' pour le médecin traitant et @code='ATTPHYS' pour le Cardiologue traitant.
        </assert>
    </rule>
